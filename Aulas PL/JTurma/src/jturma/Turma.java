...
public class Turma extends Observable {
    ...
    Iterable<Map.Entry<String, Aluno>> entrySet() {
        throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    }
    
    public String[] alunosPassam()
    {
        String[] s = new String[1];
        s[0]="";
        if(turma.isEmpty())
            return s;
        return this.turma.values().stream().filter(Aluno::passa).map(Aluno::getNome).toArray(String[]::new);
    }
}
